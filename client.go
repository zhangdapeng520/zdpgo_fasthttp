...
// Client 实现http的client
type Client struct {
	noCopy                        noCopy
	Name                          string                     // Client 客户端名称，用于User-Agent请求头
	NoDefaultUserAgentHeader      bool                       // 是否不设置UserAgent请求头
	Dial                          DialFunc                   // 回调，用于建立到主机的新连接。
	DialDualStack                 bool                       // 如果设置为true，尝试连接到ipv4和ipv6地址。
	TLSConfig                     *tls.Config                // https连接的TLS配置。
	MaxConnsPerHost               int                        // 每台主机可能建立的最大连接数。
	MaxIdleConnDuration           time.Duration              // 空闲的keep-alive连接在此期间后将被关闭。
	MaxConnDuration               time.Duration              // 长连接在此期间后将被关闭。
	MaxIdemponentCallAttempts     int                        // 幂等调用的最大尝试次数
	ReadBufferSize                int                        // 响应读取的每个连接缓冲区大小。这也限制了最大标题大小。
	WriteBufferSize               int                        // 请求写入的每个连接缓冲区大小。
	ReadTimeout                   time.Duration              // 全响应读取(包括正文)的最大持续时间。
	WriteTimeout                  time.Duration              // 整个请求写入的最大持续时间(包括正文)。
	MaxResponseBodySize           int                        // 最大反应体大小。
	DisableHeaderNamesNormalizing bool                       // 消息头名称按原样传递，没有标准化
	DisablePathNormalizing        bool                       // 路径值按原样发送，没有标准化
	MaxConnWaitTimeout            time.Duration              // 等待空闲连接的最长时间。
	RetryIf                       RetryIfFunc                // RetryIf控制在发生错误后是否应该尝试重试。
	ConnPoolStrategy              ConnPoolStrategyType       // 连接池的策略。 可以是LIFO或FIFO(默认)。
	ConfigureClient               func(hc *HostClient) error // 配置 zdpgo_fasthttp.HostClient
...
// GetTimeout 返回状态码和响应体，响应内容会替换掉dst
...
type HostClient struct {
	...
	// header names. See https://github.com/zhangdapeng520/zdpgo_fasthttp/issues/57
	// for details.
	//
...
// Do 执行给定的HTTP请求并设置相应的响应。
//
// 请求必须包含至少非零的RequestURI和完整的url(包括方案和主机)或非零主机头+ RequestURI。
//
// 该函数不遵循重定向。 使用Get*进行以下重定向。
//
// 如果resp为nil，则Response被忽略。
//
// 如果resp为nil，则Response被忽略。 所有HostClient返回ErrNoFreeConns。 主机的MaxConns连接繁忙。
//
//建议通过AcquireRequest和AcquireResponse获取req和resp在性能关键代码。
