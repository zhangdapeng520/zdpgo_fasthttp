...
import (
	...
	"github.com/zhangdapeng520/zdpgo_fasthttp/expvarhandler"
)
...
func main() {
	...
	if *vhost {
		fs.PathRewrite = zdpgo_fasthttp.NewVHostPathRewriter(0)
	}
	...
	//     in their names.
	requestHandler := func(ctx *zdpgo_fasthttp.RequestCtx) {
	...
	if len(*addr) > 0 {
		...
		go func() {
			if err := zdpgo_fasthttp.ListenAndServe(*addr, requestHandler); err != nil {
	...
	if len(*addrTLS) > 0 {
		...
		go func() {
			if err := zdpgo_fasthttp.ListenAndServeTLS(*addrTLS, *certFile, *keyFile, requestHandler); err != nil {
...
func updateFSCounters(ctx *zdpgo_fasthttp.RequestCtx) {
	...
	case zdpgo_fasthttp.StatusOK:
		fsOKResponses.Add(1)
		fsResponseBodyBytes.Add(int64(resp.Header.ContentLength()))
	case zdpgo_fasthttp.StatusNotModified:
		fsNotModifiedResponses.Add(1)
	case zdpgo_fasthttp.StatusNotFound:
